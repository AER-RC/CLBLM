...
CONTAINS !------------------ Internal subroutine -----------------------
   ...
   !--------------------------------------------------------------------
      ...
      !--- Check if the xs molecule are already exist in the line molecules
      nx = nXsMol
      do ix = 1,nXsMol
         xsNum(ix) = xsMolNum( xPrfl%molID(ix) )
          print *,'xPrfl%molID(ix),xsNum(ix) ',xPrfl%molID(ix),xsNum(ix)
         do il = 1,nLnMol
            print *,'prfl%molID(il) ',prfl%molID(il)
