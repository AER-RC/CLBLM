...
!                                                                       
!-----------------------------------------------------------------------                                                                       
      Subroutine TIPS_2003 ( molNames, temp_lbl, scor)
!-----------------------------------------------------------------------                                                                       
      USE Module_ConstParam,  ONLY: MXMOL, NT, MXISOTPL, &
                                    clblmMolNames, MaxISOTPL_smass, &
                                    molnum
      IMPLICIT NONE 
      
      character(*) ,intent(in)  :: molNames(:)
      real         ,intent(in)  :: temp_lbl
      real         ,intent(out) :: scor(:,:)  !(MXMOL,MXISOTPL)
      
      integer :: im, mol, iso, numMol,numISO
      integer :: itemp
      real    :: temp
      real    :: gi, QT
      real    :: qt_296, qt_temp
      ...
      !    The number of isotopes each molecule:
      integer ,PARAMETER :: MaxISOTOP_tips(MXMOL) = [&
        6,      11,   18,    5,    6,     4,  3,                                   &
        3,      2 ,   1 ,    2,    1,     3,  1,      2,    2,    1,               &
        2,      5 ,   3 ,    2,    1,     3,  2,      1,    3,    2,  1,           &
        1,      1 ,   3 ,    1,    1,     1,  2,      1,    2,    2,  MaxISOTPL_smass(39), &
        2,      4 ,   1 ,    1,    6,     2,  4,    MaxISOTPL_smass(47) ]
      ...
      numMol = size(molNames)
      do 110 im = 1,numMol 
         mol = molnum( molNames(im) )
         
         numISO = MaxISOTPL_smass( mol )
         if (numISO > MaxISOTOP_tips( mol ) .and. .not.any(mol==[12,14,15,16,17,22,29])) &
            STOP '--- TIPS_2003(): Number of ISO inconsistent!'

         do 110 iso= 1,numISO !yma iso_max(mol) 
...
!
      if (QT .le. 0.) then 
         Write(*,900) clblmMolNames(Mol), mol, iso, Temp, QT, gi 
  900    format(5x,A6, 2i4,                                          &
     &        '   Q(',f7.1,' K) = ',1pE13.6,5x,0pf7.1)                  
...
!-----------------------------------------------------------------------
      ...
      !--- Local variables
      INTEGER ::  I,  J  
      REAL    ::  A0, A0D1, A0D2, A0D3
      REAL    ::  A1, A1D1, A1D2, A1D3
      REAL    ::  A2, A2D1, A2D2, A2D3
      REAL    ::  A3, A3D1, A3D2, A3D3
