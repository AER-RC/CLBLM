  ...
  function getData(fname, dat) result(isError)
   ...
   subroutine check(status, varName, fatal)
     ...
     if(status /= nf90_noerr) then
       ...
       if (fatalLoc) then
         print *, 'netCDF error: ', status, ' : ', trim(nf90_strerror(status))
        call exit(1)
      else
         print *, 'netCDF WARNING: ', status, ' : ', trim(nf90_strerror(status))
         print *, 'status', status
    end if
     end if
   ...
   function inqDim(id, dimName, dimLen)
      ...
      if (present(dimLen)) then
        if (inqDim) then
          !call check( nf90_INQ_DIMLEN(id, dimId,dimLen) )
   ...
   subroutine readReal1D(id, varName, val, fatal)
      ...
      logical,        optional, intent(in) :: fatal
      if (dbg) print*, ' ncdfUtil::readReal1D ',varName
      call check(nf90_inq_varid(id, varName, varId), varName, fatal)
   ...
   subroutine readDouble1D(id, varName, val, fatal)
      ...
      integer(kind=4)                      :: varId
      logical,        optional, intent(in) :: fatal
      if (dbg) print*, ' ncdfUtil::readDouble1D ',varName
      call check(nf90_inq_varid(id, varName, varId), varName, fatal)
      call check(nf90_get_var(id, varId, val), varName, fatal)
