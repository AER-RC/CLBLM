...
!-----------------------------------------------------------------------
      ...
      real(r8)      :: V1H , V2H               ! CNTH2O
      real          :: DVH                     ! CNTH2O
      integer       :: NPTH                    ! CNTH2O
      real          :: CSH2O(N_ABSRB)          ! CNTH2O
      real          :: CFH2O(N_ABSRB)          ! CNTH2O
      real          :: sh2o(n_absrb), FH2O(n_absrb), cfor(ipts)
      double precision  :: V1ABS_wv, V2ABS_wv            ! ABSORB
      logical       :: radflag
      ...
      real         :: SH2OT0(n_absrb),SH2OT1(n_absrb), SP(n_absrb),&
...
!
      ...
      !
         if ((V2.gt.-20.0).and.(V1.lt.20000.) .and. ((xself.gt.0.).or.(xfrgn.gt.0))) then
            ...
            call mt_ckd_h2o_absco(pave,tave,h2o_fac,v1abs_wv,v2abs_wv,dvabs,&
      ...
      ! Special code just for stand alone continuum
            ...
            npth = n_absrb
            dvh = dvabs
            csh2o = sh2o/(h2o_fac*rhoave)
            cfh2o = fh2o/((1.0-h2o_fac)*rhoave)

      ! The optional multiplication by the radiation term (JRAD) and the interpolation
      ! to the final spectral grid are handled in the mt_ckd module so none of the
      ! usual code to do that in other contnm subsections is needed here.

      ! Convert absorption coefficients to OD
            cself = wk(1)*sh2o*xself
            cfor = wk(1)*fh2o*xfrgn
      ...
      ! Add OD for  AJs
            if  (icflg.eq.1) then
               if (h2o_fac.ge.1.0) then
                   cfrgn_aj = 0.
               else
                   cfrgn_aj=wk(1)*fh2o*h2o_fac/(1.0-h2o_fac)
               endif
               c = cself-cfrgn_aj
               absrb = absrb+c
            endif
...
!-----------------------------------------------------------------------
      ...
      CHARACTER*51 CINFO1(2,17),CINFO2(2,13),CINFO3(2,9),CINFO4(2,42)
      CHARACTER*40 CHEADING(3,2)

      WRITE (IPR,910) ((CINFO1(I,J),I=1,2),J=1,17)
