...
MODULE Module_Continuum

      IMPLICIT NONE
      PRIVATE
      PUBLIC :: PRCNTM, CONTNM
...
!-----------------------------------------------------------------------
      USE Module_ConstParam       ,ONLY: r8=>kind_r8, molNum, &
                                         N_ABSRB,IPTS,IPTS2,RADCN2
      USE Module_ODLAY_CommonSub  ,ONLY: RADFN
      USE Module_Spectrum         ,ONLY: XINT, pre_xint
      IMPLICIT NONE !REAL*8           (V)

      integer   ,intent(in)    :: JRAD
      real(r8)  ,intent(in)    :: V1ABS, V2ABS            ! ABSORB
      real      ,intent(in)    :: DVABS                   ! ABSORB
      integer   ,intent(in)    :: NPTABS                  ! ABSORB
      real      ,intent(inout) :: ABSRB(:)               ! ABSORB
      ...
      real      ,intent(in)    :: Wtot
      real      ,intent(in)    :: WK(60)                  ! FILHDR
      integer   ,intent(in)    :: IAERSL
      integer   ,intent(in)    :: ICFLG


      !--- Local variables
      !
      real(r8)      :: v1ss, v2ss          ! pre_xint
      integer       :: ist, last                 ! pre_xint
      real(r8)      :: V1ABSC, V2ABSC          ! CDERIV
      real          :: DVABSC                  ! CDERIV
      integer       :: NPTABSC                 ! CDERIV
      real(r8)      :: V1H                     ! CNTH2O
      real          :: DVH                     ! CNTH2O
      integer       :: NPTH                    ! CNTH2O
      real          :: CSH2O(N_ABSRB)          ! CNTH2O
      real          :: CFH2O(N_ABSRB)          ! CNTH2O
      real(r8)      :: V1C, V2C                ! XCONT
      real          :: DVC                     ! XCONT
      integer       :: NPTC                    ! XCONT
      real          :: C(6000), CP(6000)

      real ,PARAMETER :: P0=1013., T0=296.
      real ,PARAMETER :: XLOSMT=2.68675E+19

      logical ,SAVE :: firstPass = .TRUE. !if first time, print the continuum information.
      ...
      real         :: C0(n_absrb),C1(n_absrb),C2(n_absrb)
      real         :: SH2OT0(n_absrb),SH2OT1(n_absrb),FH2O(n_absrb), SP(n_absrb),&
                      CN2T0(n_absrb),FCO2(n_absrb),CT1(n_absrb),CT2(n_absrb)
      real         :: CCH0(5150),CCH1(5150),CCH2(5150)
!v128
      real         :: CN0(5150),CN1(5150),CN2(5150)
      REAL         :: ABSBSV(n_absrb)
      CHARACTER*18 :: HNAMCNT,HVRCNT

      real :: cself(ipts),cfrgn_aj(ipts)

      real ,SAVE :: XFACCO2(500)    !Correction factors for CO2 from 2000-3000 cm-1 (mt_ckd_2.5)
      real ,SAVE :: XFACREV(0:14)   !Self correction factors for 820-960 cm-1.
!v128      real ,SAVE :: XFACREV1(1:120) !Self correction factors for 2000-3190 cm-1 (mt_ckd_2.5).
      real ,SAVE :: XFAC_RHU(-1:61) !Foreign correction factors from joint RHUBC-II/RHUBC-I analysis (mt_ckd_3.0).

      INTEGER  ::  I,          I_FIX,     J
      INTEGER  ::  JFAC,       M,         NPT1,      NPT2
      INTEGER  ::  NPTO3,      N_1,       N_2,       N_S
      REAL     ::  ADJWO2,     AMAGAT
      REAL     ::  A_H2O,      A_N2,      A_O2
      REAL     ::  BETA,       BETA1,     BETA2
      REAL     ::  CFAC,       CHIO2,     CONV_CM2MOL
      REAL     ::  C_1,        C_2,       C_F,       DT
      REAL     ::  DVT1,       DVT2,      F0,        F1
      REAL     ::  FSCAL,      H2O_FAC,   HWSQ1
      REAL     ::  RAY_EXT,    RFRGN,     RFRGN_AJ
      REAL     ::  RHOAVE,     RSELF,     SFAC,      SH2O
      REAL     ::  TAU_FAC,    TC,        TFAC
      REAL(r8) ::  V0F1,       V1T1,      V1T2,      V2T1
      REAL(r8) ::  V2T2,       VDELMSQ1,  VDELSQ1,   VF1
      REAL(r8) ::  VF2,        VJ,        VMF1,      VRAYLEIGH
      REAL     ::  WCO2,       WN2,       WO2,       WO3
!ab.OrderMol      REAL     ::  WTOT
      REAL     ::  XKT,        XVRAYLEIGH
      REAL     ::  X_VMR_H2O,  X_VMR_N2,  X_VMR_O2
!v128
      real     :: f1_rhu, beta_rhu

      integer :: indH2O, indO2, indCO2, indO3
...
!
      ...
      x_vmr_h2o = wk(indH2O)/wtot !wk(1)/wtot
      x_vmr_o2  = wk(indO2)/wtot   !wk(7)/wtot
...
!
      if (iaersl.eq.5) then
         ...
         call cld_od( V1C,V2C,DVC,NPTC,c_cld,layer,xkt, V1ABS,V2ABS )
...
!=======================================================================
!
      h2o_fac  = WK(indH2O)/Wtot !WK(1)/Wtot
...
!
      if ((V2.gt.-20.0).and.(V1.lt.20000.) .and. xself.gt.0.) then      
         ...
         sp = 0.
         !sh2ot1 = 0.
!     Get 296K value for SELF coefficient
         CALL SL296 (V1C,V2C,DVC,NPTC,SH2OT0,V1ABS,V2ABS,v1ss,v2ss)
...
!     coefficents.
            ...
            CALL FRN296 (V1C,V2C,DVC,NPTC,FH2O, V1ABS,V2ABS,v1ss,v2ss )
!
            DO 24 J = 1, NPTC
               VJ = V1C+DVC* REAL(J-1)
               IF (VJ .LE. 600.) THEN
...
!
     ...
     &                                                 (1.+C_2*VF2)
               ...
               FH2O(J)=FH2O(J)*FSCAL
!
               c_f = WK(indH2O) * FH2O(J) !WK(1) * FH2O(J)
!
!********************************************
...
!
         if ((V2.gt.-20.0).and.(V1.lt.10000.) .and. xco2c.gt.0) then
            ...
            WCO2 = WK(indCO2) * RHOAVE * 1.0E-20 * xco2c       !WK(2) * RHOAVE * 1.0E-20 * xco2c
!
            CALL FRNCO2 (V1C,V2C,DVC,NPTC,FCO2,tave, V1ABS,V2ABS )
!
            DO 30 J = 1, NPTC
               VJ = V1C+DVC* REAL(J-1)
...
!
         IF (V2.GT.8920.0.AND.V1.LE.24665.0.and.xo3cn.gt.0.) THEN
            ...
            WO3 = WK(indO3) * 1.0E-20 * xo3cn   !WK(3) * 1.0E-20 * xo3cn
            CALL XO3CHP (V1C,V2C,DVC,NPTO3,CCH0,CCH1,CCH2, V1ABS,V2ABS )
!
            DT=TAVE-273.15
!
            DO 50 J = 1, NPTO3
...
!
         IF (V2.GT.27370..AND.V1.LT.40800. .and.xo3cn.gt.0.) THEN
            ...
            WO3 = WK(indO3) * 1.E-20 * xo3cn   !WK(3) * 1.E-20 * xo3cn
            TC = TAVE-273.15
            CALL O3HHT0 (V1C,V2C,DVC,NPTO3,C0, V1ABS,V2ABS )
            CALL O3HHT1 (V1T1,V2T1,DVT1,NPT1,CT1, V1ABS,V2ABS )
            CALL O3HHT2 (V1T2,V2T2,DVT2,NPT2,CT2, V1ABS,V2ABS )
!
            DO 60 J = 1, NPTO3
               C(J) = C0(J)*WO3
!
...
!
         IF (V2.GT.40800..AND.V1.LT.54000. .and.xo3cn.gt.0.) THEN
            ...
            WO3 = WK(indO3) * xo3cn  !WK(3) * xo3cn
            CALL O3HHUV( V1C,V2C,DVC,NPTO3,C0, V1ABS,V2ABS )
!
            DO 70 J = 1, NPTO3
               C(J) = C0(J)*WO3
               VJ = V1C+DVC* REAL(J-1)
...
!        Only calculate if V2 > 1340. cm-1 and V1 <  1850. cm-1
         ...
         if ((V2.gt.1340.0).and.(V1.lt.1850.).and. xo2cn.gt.0.) then
            ...
            tau_fac = xo2cn *  Wk(indO2) * 1.e-20 * amagat   !xo2cn *  Wk(7) * 1.e-20 * amagat
!
...
!
            call o2_ver_1 (v1c,v2c,dvc,nptc,c0,tave, V1ABS,V2ABS )
!
...
!
         if ((V2.gt.7536.0).and.(V1.lt.8500.).and. xo2cn.gt.0.) then
            ...
            tau_fac = xo2cn * (Wk(indO2)/xlosmt) * amagat *                 &
     &           (a_o2*x_vmr_o2+a_n2*x_vmr_n2+a_h2o*x_vmr_h2o)

!
            CALL O2INF1( V1C,V2C,DVC,NPTC,C0, V1ABS,V2ABS )
!
            DO 92 J = 1, NPTC
               C(J) = tau_fac * C0(J)
               VJ = V1C+DVC* REAL(J-1)
...
!
         if ((V2.gt.9100.0).and.(V1.lt.11000.).and. xo2cn.gt.0.) then
            ...
            CALL O2INF2( V1C,V2C,DVC,NPTC,C0, V1ABS,V2ABS )
            WO2 = xo2cn * (WK(indO2)*1.e-20) * RHOAVE  !xo2cn * (WK(7)*1.e-20) * RHOAVE
            ADJWO2 = (WK(indO2)/WTOT) * (1./0.209) * WO2  !(WK(7)/WTOT) * (1./0.209) * WO2
!
            DO 93 J = 1, NPTC
               C(J) = C0(J)*ADJWO2
               VJ = V1C+DVC* REAL(J-1)
...
!
!
            CALL O2INF3 (V1C,V2C,DVC,NPTC,C0, V1ABS,V2ABS,v1ss,v2ss )
!
            DO 94 J = 1, NPTC
               C(J) = tau_fac * C0(J)
               VJ = V1C+DVC* REAL(J-1)
...
!
         if ((V2.gt.15000.0).and.(V1.lt.29870.).and. xo2cn.gt.0.) then
            ...
            WO2 = WK(indO2) * 1.e-20 * ((pave/1013.)*(273./tave)) * xo2cn  !WK(7) * 1.e-20 * ((pave/1013.)*(273./tave)) * xo2cn
            CHIO2 =  WK(indO2)/WTOT  !WK(7)/WTOT
            ADJWO2 = chio2 * WO2
!
            CALL O2_vis( V1C,V2C,DVC,NPTC,C0, V1ABS,V2ABS )
!
            DO 96 J = 1, NPTC
...
!        Only calculate if V2 > 36000. cm-1
         ...
         if (V2.gt.36000.0 .and. xo2cn.gt.0.) then
            ...
            WO2 = WK(indO2) * 1.e-20 * xo2cn !WK(7) * 1.e-20 * xo2cn
!
            CALL O2HERZ( V1C,V2C,DVC,NPTC,C0,TAVE,PAVE, V1ABS,V2ABS )
            DO 90 J = 1, NPTC
               C(J) = C0(J)*WO2
               VJ = V1C+DVC* REAL(J-1)
!
...
!
      CALL O2FUV (V1C,V2C,DVC,NPTC,C0,V1ABS,V2ABS,v1ss,v2ss)
      DO 97 J = 1, NPTC
         VJ = V1C+DVC* REAL(J-1)
         C(J) = C0(J)*WO2
!
...
!
            CALL xn2_r (V1C,V2C,DVC,NPTC,c0,c1,Tave, V1ABS,V2ABS )
!
...
!           subroutine n2_ver_1:
!
            call n2_ver_1 (v1c,v2c,dvc,nptc,cn0,cn1,cn2,tave, V1ABS,V2ABS)
!
...
!
            call n2_overtone1 (v1c,v2c,dvc,nptc,c0, V1ABS,V2ABS)
!
...
!-----------------------------------------------------------------------
      ...
      CHARACTER*18 cnam3(9),cnam4(43)
      CHARACTER*51 CINFO1(2,15),CINFO2(2,13),CINFO3(2,9),CINFO4(2,42)
      CHARACTER*40 CHEADING(3,2)

      WRITE (IPR,910) ((CINFO1(I,J),I=1,2),J=1,15)
      WRITE (IPR,910) ((CINFO2(I,J),I=1,2),J=1,13)
...
!           123456789-123456789-123456789-123456789-123456789-1
     ...
     &     '   mt_ckd_3.2 - Cs>2000,T 1800-3500 cm-1 (Aug 2017)',       &
     &     '            AIR           0 - 20000 CM-1           ',       &
...
!           123456789-123456789-123456789-123456789-123456789-1
     ...
     &     '       O2   AIR     12990 - 13223 CM-1 ',                   &
     &     '   mt_ckd_3.1  - Mlawer and Gombos       (Jul 2017)',       &
     &     '            O2        15000 - 29870 CM-1           ',       &
     ...
     &     '  H2O self: Mlawer/Mascio, Compute tdep as T^n, n d',       &
     &     'etermined from existing 270-305K behavior(Jan 2022)',       &
     &     '  -------------------------------------------------',       &
     &     '---------------------------------------------------'/
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
      SUBROUTINE SL296( V1C,V2C,DVC,NPTC,C, V1ABS,V2ABS,v1ss,v2ss )
!-----------------------------------------------------------------------
      USE Module_ConstParam ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)

      real(r8) ,intent(out) :: V1C, V2C
      real     ,intent(out) :: DVC, v1ss,v2ss
      integer  ,intent(out) :: NPTC
      real     ,intent(out) :: C(:)
      real(r8) ,intent(in)  :: V1ABS, V2ABS

      !---Local variables
      INTEGER  :: I,  I1, I2, J

      !---Local common block
      REAL(r8) ,SAVE :: V1S,V2S
      REAL     ,SAVE :: DVS, S(2003)
      INTEGER  ,SAVE :: NPTS
   ...
   10 END DO
      ...
      DATA V1S,V2S,DVS,NPTS / -20.0, 20000.0, 10.0, 2003/
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
      SUBROUTINE SLP( V1C,V2C,DVC,NPTC,CP, V1ABS,V2ABS,v1ss,v2ss )
!-----------------------------------------------------------------------
      USE Module_ConstParam ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)
...
!               UNITLESS: Temperature exponents for SELF continuum
     ...
     &  6.275e+00,  6.414e+00,  &
      !DATA SP0001/                                                       &
     &  6.413e+00,  6.414e+00,  6.275e+00,  6.049e+00,  5.789e+00,      &
     &  5.557e+00,  5.330e+00,  5.107e+00,  4.893e+00,  4.685e+00,      &
     &  4.481e+00,  4.282e+00,  4.090e+00,  3.905e+00,  3.720e+00,      &
     &  3.548e+00,  3.379e+00,  3.215e+00,  3.056e+00,  2.903e+00,      &
     &  2.756e+00,  2.614e+00,  2.475e+00,  2.344e+00,  2.219e+00,      &
     &  2.100e+00,  1.983e+00,  1.873e+00,  1.768e+00,  1.670e+00,      &
     &  1.575e+00,  1.486e+00,  1.450e+00,  1.432e+00,  1.429e+00,      &
     &  1.431e+00,  1.428e+00,  1.458e+00,  1.517e+00,  1.570e+00,      &
     &  1.628e+00,  1.689e+00,  1.753e+00,  1.819e+00,  1.886e+00,      &
     &  1.976e+00,  2.032e+00,  2.106e+00,  2.190e+00,  2.259e+00,  &
     !DATA SP0051/                                                       &
     &  2.330e+00,  2.397e+00,  2.466e+00,  2.537e+00,  2.613e+00,      &
     &  2.675e+00,  2.746e+00,  2.819e+00,  2.893e+00,  2.970e+00,      &
     &  3.058e+00,  3.168e+00,  3.319e+00,  3.456e+00,  3.601e+00,      &
     &  3.752e+00,  3.900e+00,  4.040e+00,  4.183e+00,  4.327e+00,      &
     &  4.470e+00,  4.602e+00,  4.753e+00,  4.897e+00,  5.039e+00,      &
     &  5.214e+00,  5.409e+00,  5.608e+00,  5.782e+00,  5.917e+00,      &
     &  5.983e+00,  5.998e+00,  6.016e+00,  6.016e+00,  6.037e+00,      &
     &  6.026e+00,  6.018e+00,  6.021e+00,  6.006e+00,  6.005e+00,      &
     &  5.988e+00,  5.978e+00,  5.956e+00,  5.947e+00,  5.898e+00,      &
     &  5.910e+00,  5.880e+00,  5.842e+00,  5.819e+00,  5.772e+00,  &
     !DATA SP0101/                                                       &
     &  5.775e+00,  5.740e+00,  5.712e+00,  5.685e+00,  5.659e+00,      &
     &  5.631e+00,  5.601e+00,  5.574e+00,  5.541e+00,  5.509e+00,      &
     &  5.474e+00,  5.437e+00,  5.399e+00,  5.359e+00,  5.314e+00,      &
     &  5.261e+00,  5.205e+00,  5.146e+00,  5.075e+00,  4.997e+00,      &
     &  4.909e+00,  4.818e+00,  4.709e+00,  4.575e+00,  4.411e+00,      &
     &  4.245e+00,  4.075e+00,  3.889e+00,  3.672e+00,  3.387e+00,      &
     &  3.173e+00,  2.907e+00,  2.708e+00,  2.536e+00,  2.350e+00,      &
     &  2.273e+00,  2.196e+00,  2.178e+00,  2.201e+00,  2.238e+00,      &
     &  2.309e+00,  2.382e+00,  2.462e+00,  2.579e+00,  2.679e+00,      &
     &  2.737e+00,  2.810e+00,  2.887e+00,  2.964e+00,  2.980e+00,  &
     !DATA SP0151/                                                       &
     &  3.011e+00,  3.098e+00,  3.157e+00,  3.211e+00,  3.274e+00,      &
     &  3.388e+00,  3.475e+00,  3.527e+00,  3.618e+00,  3.772e+00,      &
     &  9.661e+00,  9.695e+00,  9.726e+00,  9.761e+00,  9.792e+00,      &
     &  9.825e+00,  9.859e+00,  9.891e+00,  9.924e+00,  9.954e+00,      &
     &  9.987e+00,  1.002e+01,  1.006e+01,  1.009e+01,  1.012e+01,      &
     &  1.016e+01,  1.019e+01,  1.022e+01,  1.026e+01,  1.029e+01,  &
      !DATA SP2001/                                                       &
     &  1.032e+01/
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
      SUBROUTINE FRN296( V1C,V2C,DVC,NPTC,C, V1ABS,V2ABS, v1ss,v2ss )
!-----------------------------------------------------------------------
      USE Module_ConstParam ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
      SUBROUTINE FRNCO2( V1C,V2C,DVC,NPTC,C,tave, V1ABS,V2ABS )
!-----------------------------------------------------------------------
      USE Module_ConstParam ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)
      ...
      !---Local common block
      REAL(r8) ,SAVE :: V1S,V2S
      REAL     ,SAVE :: DVS, S(5003)
      INTEGER  ,SAVE :: NPTS
      !COMMON /FCO2/ V1S,V2S,DVS,NPTS,S(5003)

      !---Local variables
      INTEGER :: I,  I1,  I2, J
      REAL    :: TCOR, TDEP_BANDHEAD(1196:1220)
      REAL    :: TRAT, T_EFF
...
!     of calculations in this region for IASI low PWV cases).
   ...
   10 END DO
      ...
      DATA V1S,V2S,DVS,NPTS/ -4.0, 10000.0, 2.0, 5003/
...
!-----------------------------------------------------------------------
      SUBROUTINE xn2_r( V1C,V2C,DVC,NPTC,C,fo2,Tave, V1ABS,V2ABS )
!-----------------------------------------------------------------------
      USE Module_ConstParam, ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)
      ...
      !---Local variables
      INTEGER :: I,     I1,    I2,    J
      REAL    :: SF_T,  TFAC,  T_220, T_296
      REAL    :: XN2,   XO2

      !---Local common block
      REAL(r8) ,SAVE :: V1S,V2S
      REAL     ,SAVE :: DVS, C_296(73),sf_296(73)
      INTEGER  ,SAVE :: NPTS
      REAL(r8) ,SAVE :: V1b,V2b
      REAL     ,SAVE :: DVb, C_220(73),sf_220(73)
      INTEGER  ,SAVE :: NPTb
...
!        fo2(i) = (sf_T - 1.)*(xn2**2)/(xn2*xo2)
   ...
   10 END DO
     ...
     &         1.6157,     1.6157,     1.6157/
      ...
      DATA V1b,V2b,DVb,NPTb/ -10., 350., 5.0, 73 /
...
!     Alvarado et al. (2012).
!-----------------------------------------------------------------------
      subroutine n2_ver_1 (v1c,v2c,dvc,nptc,c,c1,c2,T, V1ABS,V2ABS )
!-----------------------------------------------------------------------
      USE Module_ConstParam, ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8 (v)
      ...
      !---Local common block
      REAL(r8) ,SAVE :: V1S,V2S
      REAL     ,SAVE :: DVS, xn2_272(230), xn2_228(230), a_h2o(230)
      INTEGER  ,SAVE :: NPTS
...
!     broadener.
!-----------------------------------------------------------------------
      subroutine n2_overtone1 (v1c,v2c,dvc,nptc,c, V1ABS, V2ABS)
!-----------------------------------------------------------------------
      USE Module_ConstParam, ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8 (v)
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
      SUBROUTINE XO3CHP( V1C,V2C,DVC,NPTC,C0,C1,C2, V1ABS,V2ABS )
!-----------------------------------------------------------------------
      USE Module_ConstParam  ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)
      ...
      !---Local common block
      REAL(r8) ,SAVE :: V1S,V2S
      REAL     ,SAVE :: DVS, X(3150),Y(3150),Z(3150)
      INTEGER  ,SAVE :: NPTS
...
!         Remove radiation field from diffuse ozone
   ...
   10 END DO
      ...
      DATA V1S,V2S,DVS,NPTS /8920.0, 24665.0, 5.0, 3150/
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
      SUBROUTINE O3HHT0( V1C,V2C,DVC,NPTC,C, V1ABS,V2ABS )
!-----------------------------------------------------------------------
      USE Module_ConstParam ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)
...
!     RADIATION FLD REMOVED FROM DIFFUSE OZONE
   ...
   10 END DO
      ...
      DATA V1S,V2S,DVS,NPTS /27370., 40800., 5., 2687/
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
      SUBROUTINE O3HHT1( V1C,V2C,DVC,NPTC,C, V1ABS,V2ABS )
!-----------------------------------------------------------------------
      USE Module_ConstParam ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
      SUBROUTINE O3HHT2( V1C,V2C,DVC,NPTC,C, V1ABS,V2ABS )
!-----------------------------------------------------------------------
      USE Module_ConstParam  ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
      SUBROUTINE O3HHUV( V1C,V2C,DVC,NPTC,C, V1ABS,V2ABS )
!-----------------------------------------------------------------------
      USE Module_ConstParam ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)
...
!        RADIATION FLD REMOVED FROM U.V.    OZONE
   ...
   10 END DO
      ...
      DATA V1S,V2S,DVS,NPTS /40800., 54000., 100., 133/
...
!-----------------------------------------------------------------------
      subroutine o2_ver_1( v1c,v2c,dvc,nptc,c,T, V1ABS,V2ABS )
!-----------------------------------------------------------------------
      USE Module_ConstParam  ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8 (v)
      ...
      !--- Local common block
      REAL(r8) ,SAVE :: V1S,V2S
      REAL     ,SAVE :: DVS, XO2(103),XO2T(103)
      INTEGER  ,SAVE :: NPTS
...
!-----------------------------------------------------------------------
      SUBROUTINE O2INF1( V1C,V2C,DVC,NPTC,C, V1ABS,V2ABS )
!-----------------------------------------------------------------------
      USE Module_ConstParam ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)
   ...
   10 END DO
      ...
      DATA V1S,V2S,DVS,NPTS /7536.000,8500.000,   2.000,  483/
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
      SUBROUTINE O2INF2( V1C,V2C,DVC,NPTC,C, V1ABS,V2ABS )
!-----------------------------------------------------------------------
      USE Module_ConstParam ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)
      ...
      !---Local variables
      INTEGER  :: J
      REAL     :: DAMP1, DAMP2,   DV1,  DV2
      REAL     :: DVS,   O2INF
      REAL(r8) :: V1S,   V2S
      REAL(r8) :: VJ

      REAL(r8) ,SAVE :: V1_OSC,  V2_OSC
      REAL     ,SAVE :: HW1, HW2
      REAL     ,SAVE :: S1, S2
...
!
!-----------------------------------------------------------------------
      SUBROUTINE O2INF3 (V1C,V2C,DVC,NPTC,C, V1ABS, V2ABS, v1ss, v2ss)
!-----------------------------------------------------------------------
      USE Module_ConstParam ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)
...
!
      ...
      DATA V1S,V2S,DVS,NPTS /12961.5, 13221.5, 1.0, 261/
...
!-----------------------------------------------------------------------
      SUBROUTINE O2_vis( V1C,V2C,DVC,NPTC,C, V1ABS,V2ABS )
!-----------------------------------------------------------------------
      USE Module_ConstParam  ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)
   ...
   10 END DO
      ...
      DATA V1s,V2s,DVs,NPTs /15000.0, 29870.0, 10.0,  1488/
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
      SUBROUTINE O2HERZ( V1C,V2C,DVC,NPTC,C,T,P, V1ABS,V2ABS )
!-----------------------------------------------------------------------
      USE Module_ConstParam ,ONLY: r8=>kind_r8
      IMPLICIT NONE !REAL*8           (V)
...
!     --------------------------------------------------------------
!
      SUBROUTINE O2FUV (V1C,V2C,DVC,NPTC,C,V1ABS,V2ABS,v1ss,v2ss)
         !
         USE Module_ConstParam ,ONLY: r8=>kind_r8
         IMPLICIT NONE !REAL*8           (V)
         ...
         !
            ...
            DATA V1S,V2S,DVS,NPTS /56740.0, 86960.0, 20.0,  1512/
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
      subroutine cld_od( V1C,V2C,DVC,NPTC,C,layer,xkt, V1ABS,V2ABS )
!-----------------------------------------------------------------------
      USE Module_ConstParam       ,ONLY: r8=>kind_r8
      USE Module_ODLAY_CommonSub  ,ONLY: RADFN
      USE Module_Config           ,ONLY: IPR
      ...
      !--- Local variables
      !
      integer ,PARAMETER :: n_lyr=200, n_cld=500

      integer  ,SAVE :: N_FREQ                      ! CLD_RD
      real     ,SAVE :: CLOUDODLAYER(N_LYR,N_CLD)   ! CLD_RD
      real(r8) ,SAVE :: V_CLOUD_FREQ(N_CLD)         ! CLD_RD

      CHARACTER*55 :: IN_CLD_FILE
      INTEGER      :: I,    ILO,   I_LAYER(N_LYR)
      INTEGER      :: J,    L,     N_LAYER
      REAL         :: DVS,  PRES_LAYER_DUM(N_LYR)
      REAL(r8)     :: V_CNTNM(N_LYR),  V_M
